...
import { TextField, Table, TableBody, Typography, TableCell, TableContainer, TableHead, TableRow, Grid, FormControl, InputLabel, Select, MenuItem, Button, Box, Dialog, DialogContent, DialogActions, DialogContentText, IconButton, } from '@mui/material'
...
const Level2casedetails = () => {
  ...
  const remarksRfiRef = useRef<HTMLInputElement>(null);
  ...
  useEffect(() => {
    if (selectedStatusRfi && remarksRfiRef.current) {
      remarksRfiRef.current.focus();
    }
  }, [selectedStatusRfi]);
  ...
  const handleRIFIconClick = (index: number) => {
    if (!pendingRif || !Array.isArray(pendingRif)) {
      console.error('pendingRif is undefined or not an array');
      return;
    }
  ...
  const fetchStatus = async () => {
    try {
      const filteredStatuses = await status.getPepStatus();
                    ...
                    <Table size="small" stickyHeader aria-label="sticky table" style={{ margin: '0 auto' }}>
                      <TableHead sx={{ backgroundColor: '#cccdd1' }}>
                        <TableRow className="tableHeading">
                          ...
                          <TableCell sx={{ backgroundColor: '#D3D3D3', padding: '4px', }}>
                            <strong>SearchName</strong></TableCell>
                          <TableCell sx={{ backgroundColor: '#D3D3D3', padding: '4px' }}>
                            <strong >Name</strong></TableCell>
                          <TableCell sx={{ backgroundColor: '#D3D3D3', padding: '4px' }}>
                            <strong >Score</strong></TableCell>
                          <TableCell sx={{ backgroundColor: '#D3D3D3', padding: '4px' }}>
                            <strong >Remark</strong></TableCell>
                          <TableCell sx={{ backgroundColor: '#D3D3D3', padding: '4px' }}>
                            <strong >Action</strong></TableCell>
                        </TableRow>
                      </TableHead>
                      <TableBody>
                        {pendingcase.length === 0 ? (
                          ...
                          <TableRow key={index} style={{ height: '32px' }}>
                            ...
                            <TableCell onClick={() => handleTableRowClick(Number(alert.criminalId))}
                              style={{
                                cursor: 'pointer',
                                color: '#3F51B5',
                              ...
                              <span>
                                {String(alert.criminalName).charAt(0).toUpperCase() + String(alert.criminalName).slice(1)}
                              </span>
                            </TableCell>
                            <TableCell style={{ padding: '4px', }}><span>{alert.matchingScore}</span></TableCell>
                            <TableCell style={{ padding: '4px', }}><span>{alert.remark ? alert.remark.charAt(0).toUpperCase() + alert.remark.slice(1) : 'Not Available'}</span></TableCell>
                    ...
                    <Table size="small" stickyHeader aria-label="sticky table" style={{ margin: '0 auto' }} >
                      ...
                      <TableBody>
                        {pendingRif.length === 0 ? (
                          ...
                          <TableRow key={index} style={{ height: '32px' }}>
                            <TableCell style={{ padding: '4px', }}><span>{index + 1}</span></TableCell>
                            <TableCell style={{ padding: '4px', }}><span>{alert.name.charAt(0).toUpperCase() + alert.name.slice(1)}</span></TableCell>
                            <TableCell onClick={() => handleTableRowClick(Number(alert.criminalId))}
                              style={{
                                cursor: 'pointer',
                                color: '#3F51B5',
                                textDecoration: 'underline',
                                padding: '4px',
                              }}><span>{alert.hitName.charAt(0).toUpperCase() + alert.hitName.slice(1)}</span></TableCell>
                            <TableCell style={{ padding: '4px', }}><span>{alert.score}</span></TableCell>
            ...
            >
              <span style={{ marginLeft: '3%', marginTop: '1%', fontSize: 'Large', fontWeight: 'bold' }}>
                {selectedCourierTrackerRfi.name
                  ? selectedCourierTrackerRfi.name.charAt(0).toUpperCase() + selectedCourierTrackerRfi.name.slice(1)
                  : 'Not Available'}
              </span>
              <span style={{ marginLeft: '3%' }}>
                Search Name: {selectedCourierTrackerRfi.searchName
                  ? selectedCourierTrackerRfi.searchName.charAt(0).toUpperCase() + selectedCourierTrackerRfi.searchName.slice(1)
                  : 'Not Available'}
              </span>
              <span style={{ marginLeft: '3%' }}>
                Matching Score: {selectedCourierTrackerRfi.score || 'Not Available'}
              </span>
              <ul className="timeline">
                {remarkDetails && remarkDetails.length > 0 ? (
                  (() => {
                        ...
                        <li className="list active" key={index}>
                          <div className="list-content">
                            <div className="details">
                              ...
                              <span className="status-title" style={{ color: '#080807', fontSize: 'small' }}>
                                {detail.remark
                                  ? detail.remark.charAt(0).toUpperCase() + detail.remark.slice(1)
                                  : 'Not Available'}
                              </span>
                            </div>
                          </div>
