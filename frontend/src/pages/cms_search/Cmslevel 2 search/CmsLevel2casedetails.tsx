...
const Level2casedetails: React.FC<{
  ...
  // const getPendingOneRemarkDetails= async(criminalId: any, hitdataId: any, levelId: any, statusId: any)=>{
  //   try {
  //     const filteredStatuses = await authService.getPendingOneRemarkDetails(criminalId,hitdataId,1,1)
  //     setStatusData(filteredStatuses)
  //   } catch (error) {
  //     console.error("Error fetching types:", error);
  //   }
  ...
  //   getPendingOneRemarkDetails(selectedSearchResult);
  ...
  const handleIconClick = (index: number) => {
    ...
    // Fetch the selected row data based on the current index
    const selectedSearchResult = pendingcase[currentIndex]; // or pendingRif[currentIndex]
    
    if (selectedSearchResult) {
      // Assuming selectedSearchResult contains properties: criminalId, hitId, levelId, statusId
      const { criminalId, hitId, levelId, statusId } = selectedSearchResult;
  
      // Call getPendingOneRemarkDetails with the correct parameters
      getPendingOneRemarkDetails(criminalId, hitId, levelId, statusId);
  
      // Update the UI state based on the selected data
      setSelectedStatus(existingAction);
      setRemarks(existingRemarks);
      setSelectedRow(currentIndex);
      setIsRemarksDialogOpen(true);
