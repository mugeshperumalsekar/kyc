...
import CmsLevelsFlow from './CmsLevelsFlow';
...
function LevelFlow() {
  ...
  const handleoneRemark = async (hitId: any) => {
    try {
        const response = await authService.getcmsRemarkending(hitId);
    ...
    <>
      <Box sx={{ display: 'flex' }}>
        ...
        <Box component="main" sx={{ flexGrow: 1, p: 3, m: 4 }}>
          ...
          {levelStatus[0]?.levelId === 1 && <CmsLevelsFlow />}
                ...
                <TableBody>
                  {loading ? (
                    ...
                    pendingAlert.map((alert, index) => (
                      <React.Fragment key={alert.cmsId}>
                        <TableRow key={index} style={{ height: '32px' }}>
                          ...
                          <TableCell style={{ padding: '4px' }}>
                            <a
                              ...
                              onClick={(e) => {
                                e.preventDefault();
                                handleTableRowClick(Number(alert.criminalId),  alert.recordTypeId,);
                              }}
                              style={{
                                cursor: 'pointer',
                                color: '#1677FF',
