import { Card, Typography, Button, TableContainer, Table, TableHead, TableRow, TableCell, TableBody, Box, Modal, TablePagination, IconButton, Dialog, DialogActions, DialogTitle, DialogContentText, DialogContent, FormControl, InputLabel, Select, MenuItem, Grid, TextField, SelectChangeEvent } from '@mui/material';
import React, { useState, useEffect } from 'react';
import VisibilityIcon from '@mui/icons-material/Visibility';
import ClearIcon from '@mui/icons-material/Clear';
import ArrowDropUpIcon from '@mui/icons-material/ArrowDropUp';
import SearchApiService from '../../data/services/pep_search/search-api-service';
import { CmsGetSearchDetails, CmsHitSearchData } from '../../data/services/pep_search/search-payload';
...
function CmsSearchDetails() {

    const [cmsData, setCmsData] = useState<CmsGetSearchDetails[]>([]);
    const [hitRecordData, setHitRecordData] = useState<CmsHitSearchData[]>([]);
    ...
    //     try {
    //         const data = await searchApiService.getCmsSearchDetails();
    //         setCmsData(data);
    //     } catch (error) {
        ...
        <>
            <Box sx={{ display: 'flex' }}>
                ...
                <Box component="main" sx={{ flexGrow: 1, p: 3, justifyContent: 'center' }}>
                    <h4 style={{ marginTop: '6%', textAlign: 'center' }}>CRIMINAL SEARCH DETAILS</h4>
                    ...
                    <Card sx={{ padding: '16px', boxShadow: 3, marginTop: 2, width: '50%', marginLeft: '25%' }}>
                        <TableContainer>
                            <Table size="medium">
                                ...
                                <TableBody>
                                    {cmsData && cmsData.slice().reverse().slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, index) => (
                                        <React.Fragment key={row.id}>
                                            ...
                                            {expandedRowId === row.id && hitRecordData.length > 0 && (
                                                <TableRow>
                            ...
                            <TablePagination
                                rowsPerPageOptions={[5, 10, 20, 200]}
                                component="div"
                                count={cmsData ? cmsData.length : 0}
                                page={page}
...
export default CmsSearchDetails;