...
function SancSearchDetails() {
    ...
    const fetchSancData = async () => {
        try {
            ...
            // Filter the data to only include rows where kycId is not null or empty
            const filteredData = data.filter((row: { kycId: string | null; }) => row.kycId !== null && row.kycId !== "\\N" && row.kycId !== "");

            setSanctionData(filteredData); // Update state with filtered data
