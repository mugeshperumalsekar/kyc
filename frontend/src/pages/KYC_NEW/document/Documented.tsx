...
function Document() {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        let isAnyFileUploaded = false;

        for (const bank of banks) {
            for (const category in bankStates[bank.answer]) {
                if (bankStates[bank.answer][category].checked) {
                    isAnyCheckboxChecked = true;
                }
                if (bankStates[bank.answer][category].file.length > 0) {
                    isAnyFileUploaded = true;
                }
            }
        }

        if (!isAnyCheckboxChecked || !isAnyFileUploaded) {
            setSnackbarMessage('Please select atleast one checkbox and upload atleast one document.');
        ...
        try {
            for (const bank of banks) {
                ...
                console.log('BankState for bank:', bank.answer, bankState);

                for (const category in bankState) {
                    const file = bankState[category].file[0];
                    console.log(`Processing category: ${category}, File selected:`, file ? file.name : 'No file selected');

                    const isChecked = bankState[category].checked ? 1 : 0;
                    console.log(`Checkbox state for category ${category}: ${isChecked}`);
                    ...
                    if (file && file.name) {
                        ...
                        if (kycIdNumber && pathId) {
                            console.log(`Uploading file for bank: ${bank.answer}, category: ${category}, KYC ID: ${kycIdNumber}, Path ID: ${pathId}`);
                                    ...
                                    {['pep', 'san', 'crm', 'aml'].map((category) => (
                                        <div key={category} style={{ display: 'flex', alignItems: 'center' }}>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                <IconButton onClick={() => handleChooseImagesClick(bank.answer, category)} color={bankStates[bank.answer]?.[category]?.file.length > 0 ? 'success' : 'primary'}>
                                                    <CloudUploadIcon />
                                                </IconButton>
