...
class CountryApiService {
    ...
    getCountryOptions = async () => {
        try {
            const response = await this.httpClientWrapper.googleget('/api/v1/Country');
            return response;
        } catch (error) {
            throw error;
        }
    ...
    updateCountry = async (id: number, payload: CountryEditPayload) => {
        try {
            const response = await this.httpClientWrapper.googleput(`/api/v1/Country/${id}`, payload);
    ...
    blockCountry = async (id: number) => {
        try {
            const response = await this.httpClientWrapper.googleput(`/api/v1/Country/${id}/block`);
            const data = response.data;
            return data;
        } catch (error) {
    ...
    unblockCountry = async (id: number) => {
        try {
            const response = await this.httpClientWrapper.googleput(`/api/v1/Country/${id}/unblock`);
            const data = response.data;
            return data;
        } catch (error) {
            console.error("StateApiService deleteState() error:", error);
