...
class LevelApiService {
    ...
    CreateLevel = async (payload: LevelPayload) => {
        try {
            const response = await this.httpClientWrapper.ALpost3('/api/v1/Level/CreateLevelRequest', payload);
            return response;
        } catch (error) {
            throw error;
        }
    ...
    fetchLevelById = async (id: number) => {
        try {
            const response = await this.httpClientWrapper.ALget3(`/api/v1/Level/${id}`);
